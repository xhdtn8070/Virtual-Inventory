...
public class VirtualInventoryManager {
    ...
    public VirtualInventoryManager(VirtualInventoryPlugin virtualInventoryPlugin, File folder) {
        ...
        if (!folder.exists()) {
            folder.mkdir();
            Common.log("&amkdir " + folder.getAbsolutePath());
        }
    }
    ...
    public VirtualInventory createInventory(Player player, String inventoryName) {
        ...
        if (inventory == null || inventory.isDelete()) {
            if (getMaxInventory() < getAmountInventory(player)) {
                ...
                Common.tell(player, this.virtualInventoryPlugin.messageConfig.getInventory().getCreateError());
                return null;
            }
            String playerName = player.getName().toLowerCase();
            String json = playerName + "." + inventoryName;
            ...
            String title = this.virtualInventoryPlugin.messageConfig.getInventory().getTitle();
        ...
        Common.tell(player, this.virtualInventoryPlugin.messageConfig.getInventory().getAlreadyExists());
        return inventory;
    }
    ...
    public int getMaxInventory() {
        return this.virtualInventoryPlugin.pluginConfig.getMaxInventory();
    }

    public void load() throws IOException, ClassNotFoundException {
        boolean loadMessaging = this.virtualInventoryPlugin.pluginConfig.isLoadMessaging();
        String loadMessage = this.virtualInventoryPlugin.pluginConfig.getLoadMessage();
    ...
    public void saveFile(File file, VirtualInventory virtualInventory, boolean force) throws IOException {
        ...
        if ((!force) && !virtualInventory.isModify()) {
             ...
             */
            if (virtualInventory.getCount() >= this.virtualInventoryPlugin.pluginConfig.getSaveOncePerSeveralTimes()) {
    ...
    public void saveAll(boolean force) throws IOException {
        ...
        if (this.virtualInventoryPlugin.pluginConfig.isSaveMessaging()) {
            Common.broadcast(this.virtualInventoryPlugin.pluginConfig.getSaveMessage());
        } else {
            Common.log(this.virtualInventoryPlugin.pluginConfig.getSaveMessage());
        }
    ...
    public void removeInventory(Player player, String inventoryName) {
        ...
        if (list == null) {
            Common.tell(player, this.virtualInventoryPlugin.messageConfig.getInventory().getNoExists());
            return;
        }
        int index = -1;
        ...
        Common.tell(player, this.virtualInventoryPlugin.messageConfig.getInventory().getRemove());
    }
}
